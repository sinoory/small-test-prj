...
bool SrafIpcListener::Send(IPC::Message* message) {
  LOG(INFO)<<__func__<<" msg line="<<IPC_MESSAGE_ID_LINE(message->type())
      <<" msg class="<<IPC_MESSAGE_ID_CLASS(message->type())
      <<" reply_for="<<message->sync_msg_reply_for_msg;
  if (_sender) {
...
void SrafIpcThread::initChannal(EChannalType t,std::string channal_name,SrafIpcListener* listener){
  ...
  _listen_thread->message_loop()->PostTask(FROM_HERE, base::Bind(&SrafIpcThread::initChannal_, base::Unretained(this)));
}
